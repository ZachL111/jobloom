...
`jobloom` is centered on backend services work in Elixir. The technical target is: Schedule actor-style jobs with retry policy and queue inspection.
...
The score is deterministic. The current threshold is 153. Risk has a penalty of 7, latency has a penalty of 2, and weight contributes a bonus of 5.
